    ...
    def __forward(
        ...
        for operation in executing_order[: last_idx]:
            ...
            try:
                ...
                # inputs = [var if var.dtype!=torch.half else var.float() for var in inputs ]
                # print(operation,[var.name for var in operation.inputs],[var.value.dtype for var in operation.inputs])
                # print([var.dtype for var in inputs])
