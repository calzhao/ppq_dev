...
do
    ...
    do
        CUDA_VISIBLE_DEVICES='4,5,6,7' python ./ppq/samples/LLM/llm_eval.py --model $model --task $task --method cosine >wiki_llama_cosine.out
        if [ $? -ne 0 ]; then
...
do
    for task in wikitext lambada hellaswag piqa
    do
        CUDA_VISIBLE_DEVICES='4,5,6,7' python ./ppq/samples/LLM/llm_eval.py --model $model --task $task --method cosine >wiki_opt_cosine.out
        if [ $? -ne 0 ]; then
            echo "Error: Python command $model $task failed."
        fi
    done
